...
package org.osmf.player.media
{
	...
	import at.matthew.httpstreaming.HTTPStreamingM3U8NetLoaderAdapter;
	...
	public class StrobeMediaFactory extends DefaultMediaFactory
	{
		...
		{	
			...
			var m3u8Loader:M3U8Loader= new M3U8Loader(this);
			addItem 
				(new MediaFactoryItem
					( "at.matthew.httpstreaming.m3u8"
						, m3u8Loader.canHandleResource
						, function():MediaElement {
							return new M3U8Element(null, m3u8Loader);
						}
					)
				);
			...
			if (configuration.optimizeBuffering || configuration.optimizeInitialIndex)
			{
				CONFIG::FLASH_10_1
				{
					var m3u8NetLoaderAdapter:HTTPStreamingM3U8NetLoaderAdapter = new HTTPStreamingM3U8NetLoaderAdapter(playbackOptimizationManager);
					addItem(new MediaFactoryItem(
						"at.matthew.httpstreaming"
						, m3u8NetLoaderAdapter.canHandleResource
						, function():MediaElement {
							return new VideoElement(null, m3u8NetLoaderAdapter);
						}
					));
				...
				var m3u8NetLoader:HTTPStreamingM3U8NetLoader = new HTTPStreamingM3U8NetLoader();
				addItem(new MediaFactoryItem(
					"at.matthew.httpstreaming"
					, m3u8NetLoader.canHandleResource
					, function():MediaElement {
						return new VideoElement(null, m3u8NetLoader);
					}
				));
			}
		}
